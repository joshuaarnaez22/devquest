...
| **Combined maximum** | **+48% HP** *or* **+30% damage**, never both at full | Slots force the choice |
...
function sectionsUnlockedBy(world: WorldManifestEntry, isLastShipped: boolean): PortfolioSectionId[] {
...
2. Reward exploration with *capability*, not numbers.
...
**The no-strictly-better rule (G4):** every charm must be the best choice in *some* situation.
...
Shards are rare (17 in the whole game), so their collection is a *moment*:
