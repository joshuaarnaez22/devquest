...
An individual enemy may be strong. The _encounter_ — the enemy plus its placement plus the terrain — is what gets balanced. A Golem in an open room and a Golem in a corridor are different fights, and the level designer owns that difference.
...
- **Hearing is omnidirectional but short** (typically 48–60 px) and _does_ pass through walls. This models "it heard you land" and prevents cheesing enemies by standing behind a thin wall.
...
**Design question:** _Can you time a swing?_
...
**Design question:** _Can you react under pressure?_
...
**Design question:** _Can you fight what will not hold still?_
...
**The "taken 2 hits" trigger is what makes the Yokai frustrating in the right way.** You cannot simply corner it and combo. It escapes, and you must re-engage. The 2-hit threshold means a Samurai's first two combo hits land, the third whiffs. This is deliberate: it teaches the player to use hit 3's 180° arc _after_ the teleport, anticipating where it will appear.
...
**Design question:** _Can you reach the back line?_
...
**Design question:** _Can you break through?_
...
**Design question:** _Can you find the opening?_

140 HP, 90 poise, immune to knockback, and slow. The Golem cannot be staggered by normal attacks and cannot be outfought by pressure. It must be _outmanoeuvred_. It is the roster's statement that not every problem is solved by attacking harder.
...
**Design question:** _Can you use everything you have learned?_
...
const TIER_MULTIPLIERS = {
  ...
  elite: { hp: 2.4, damage: 1.5, speed: 1.2, poise: 1.6, coins: 3.4, armour: +0.1 },
} as const;
...
export class Enemy extends Entity implements Poolable {
  ...
  onDespawn(): void {
    for (const b of this.behaviours) b.cleanup?.(this.ctxFor(b));
  }
}
```
...
| _*Pathfinding (A*)_*                          | Levels are side-scrolling corridors. Ledge sensing and gap jumping cover every case. A* would be expensive and unnecessary |
