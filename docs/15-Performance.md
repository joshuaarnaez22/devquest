...
bus.on('combat:hit', p => { this.lastHit = p; });

// ✓ CORRECT — copy what you need.
bus.on('combat:hit', p => { this.lastHitDamage = p.damage; });
```
...
This is the highest-value dev-facing feature in the project. A developer who can *see* a spike the moment it appears fixes it in the same session; one who discovers it three weeks later in a CI report spends a day bisecting.
