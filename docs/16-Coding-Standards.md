...
export type BehaviourIntent =
  { kind: 'none' } | { kind: 'move'; vx: number } | { kind: 'requestAttack'; attackId: string };
    ...
    rules: {
      ...
      'no-restricted-properties': [
        'error',
        {
          object: 'Math',
          property: 'random',
          message: 'Use Registry.get("rng"). Determinism enables replays.',
        },
        {
          object: 'this.add',
          property: 'text',
          message: 'Use BitmapText. See 04-Art-Direction §9.2.',
        },
      ],
...
export function getBehaviour(id: BehaviourId): Behaviour {
  ...
  if (!b)
    throw new Error(`Unknown behaviour: ${id}. Registered: ${[...REGISTRY.keys()].join(', ')}`);
  return b;
}
```
...
| Two enemy types share 80% of their code     | _Now_ extract the shared framework  |
