...
| 5 | **Gorgon's Spire** | Castle *(pending, `05` §9.1)* | Fairy Tale (storm) | `#3a1d4d` @ 0.28 | Timed gate sequences | Wall turrets, petrify zones | Gorgon |
...
**Rule:** every `GAP_L` or `GAP_XL` on a *required* path is a bug. `check-hero-parity.ts` fails the build.
...
Beat 1 requires that failure is *free*. Concretely:
...
| Object | Parameters | Behaviour |
|---|---|---|
| *(player lantern)* | radius 64 px, always on | A soft radial light centred on the player |
...
for (const b of this.litBraziers) this.lightMask.erase(this.radialGradient, b.x, b.y, b.radius / 64);
...
onewayLayer.forEachTile(t => { if (t.index !== -1) t.setCollision(false, false, true, false); });
...
*Green Zone tileset · Nature backgrounds · ambient `#8bb4d4` @ 0.10 · Skeletons*
...
| 7 | — | **Enemy demonstration.** A Skeleton patrols on a ledge *below* the player's path, visible for 4 s. Cannot reach the player |
...
*Autumn Forest tileset · Fairy Tale backgrounds · ambient `#d4813f` @ 0.18 · Werewolves*
...
*Forbidden Graveyard tileset · Fairy Tale night · ambient `#1d2f4a` @ 0.35 · Yokai, Witches*
...
*Crystal Cave tileset · custom gradient backdrop · ambient `#0f1a2b` @ 0.40 · Orcs, Golems*
...
*Castle tileset (pending) · Fairy Tale storm · ambient `#3a1d4d` @ 0.28 · everything*
...
export interface TiledObject {
  ...
  readonly properties?: readonly { readonly name: string; readonly type: string; readonly value: unknown }[];
}
```
...
export const GAP = {
  STEP: 16, HOP: 24, GAP_S: 32, GAP_M: 40,
  GAP_L: 56, GAP_XL: 64, GAP_NINJA: 96,
} as const;

export const HEIGHT = {
  LEDGE_S: 16, LEDGE_M: 24, LEDGE_L: 26,
  LEDGE_XL: 40, SHAFT: 48, LEDGE_NINJA: 56,
} as const;

export const CLEARANCE = {
  CEIL_MIN: 32, CEIL_JUMP: 64, CEIL_COMBAT: 80,
  CORRIDOR_MIN: 32, CRAWL: 20,
} as const;
  ...
  for (const region of mainRegions) {
    for (const gap of detectGaps(level.terrain, region.bounds)) {
      ...
      if (gap.width > assisted) {
        fail(`${level.id}: main-path gap ${gap.width}px at (${gap.x},${gap.y}) `
...
registerObjectType('zipline', (obj, ctx) => {
  ...
  ctx.entities.add(new Zipline(ctx.scene, path, {
