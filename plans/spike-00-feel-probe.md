...
```ts
const GRAVITY_Y = 900, MAX_FALL = 300;
const FALL_MULT = 1.35, APEX_MULT = 0.70, APEX_THRESHOLD = 40;
const RUN_SPEED = 90, GROUND_ACCEL = 900, GROUND_DECEL = 1200;
const AIR_ACCEL = 600, AIR_DECEL = 400, TURN_BOOST = 1.8;
const JUMP_V = -240, JUMP_CUT = 0.45;
const COYOTE_MS = 100, BUFFER_MS = 120;
const DASH_SPEED = 260, DASH_MS = 150, DASH_CD_MS = 500;
```
...
| 7 | Dash — 260 px/s for 150 ms, gravity off, cooldown from *start* | §5.5 |
...
| Does `GAP_M` (40 px) feel like a *standard* gap or a *challenge*? | |
